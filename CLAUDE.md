...
- Disconnects: the relay fires `peer_left` only on a real socket close. The display additionally runs a 1 Hz liveness loop (`display/Net.js`): phones ping at 1 Hz, a seat silent past 3 s is dropped mid-game (same path as `peer_left`; any traffic restores it), and a relay self-heartbeat forces a reconnect when the display's own socket is half-dead.
