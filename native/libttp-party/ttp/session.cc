...
Value lobby_snapshot(const Value& input, const Value& chooser) {
  ...
  // A 'random' pick's run length: 0 endless, else that many races. Passthrough
  // like every other pick field — the select-mode walk (runtime/ttp_net.cc
  // normRandomRaces, over protocol.h RANDOM_RACES_*) already clamped it before
  // the snapshot is composed.
