 ...
 * the random pick's SHUFFLE BAG (page RNG, not sim state — the walks below ask
 ...
 *   arm-create-watchdog {delayMs}            arm it; on expiry call
 ...
 * only then dial fresh. That order is load-bearing. */
 ...
 * select_mode, ping, default -> game-message. `isSignal` is the shell's "the
 * fastlane consumed this as an RTC signal" — the walk then stops after the
 * liveness stamp, exactly where the old shell returned.
 ...
 * internal — a reset also happens under created/joined, BEFORE the shell's
 * timer guard can matter, so a heartbeat left in flight by a dropped socket
 * cannot survive into the fresh connection), then on a sweep the expiry drops,
 * the active-order re-sync and the abandoned-race deadline, in that order on
 ...
 * with ttp_net_on_peer_message_json calling this same walk internally. */
TTP_ABI const char* ttp_net_on_seen_json(int roomHandle, const char* peerIdJson, double nowMs);

/* The drained hostchange event's body: clear the promoted host's ready flag
 * (their footer button is "Start race", so it could never be cleared), then
 * announce. */
TTP_ABI const char* ttp_net_host_change_apply_json(int roomHandle);

/* The drained statechange event's body: restamp connected seats on COUNTDOWN
 * (never a blanket clear-disconnected — it would orphan a grace-pending seat's
 * QR), free disconnected seats on LOBBY, clear-standings, publish. */
