...
class GameCoordinator(
    ...
    fun displayReady() {
        try {
            ...
            // next scene.
            // The asset map is fresh, and so is the renderer's bake — a
            // destroyed surface took both. Nothing on this side has to be told:
            // the memo [SceneStaging.materials] clears is the only mirror left,
            // and the bake's residency is answered by the engine that holds it.
    ...
    private fun paintHUD(slots: List<DisplayHost.HudSlot>) {
        ...
        // AUTHORED canvas — 1920 by the window's own aspect — and by nothing
        // the render scale can move. The buffer width used to be the divisor,
        // and it reached Compose by a road Compose could not watch — see
        // TtpTheme.toAuthored for what that cost.
        val authoredH = display.authoredHeight()
        val rects = display.cellRects().map { it.toAuthored(authoredH) }
