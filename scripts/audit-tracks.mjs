...
//
// `auditTracks()` is the same audit as an importable function, so the gate that
// runs it on every `npm test` (tests/track-geometry.test.js) shares these rules
// rather than restating them. Printing and the exit code stay here.
...
// { trackName: [finding, ...] } for every track that has any; clean tracks are
// absent, so an empty object IS the pass condition.
export function auditTracks() {
  ...
  for (const [name, def] of Object.entries(ALL_TRACKS)) {
    ...
    if (rows.length) found[name] = rows;
  }
  return found;
}

// CLI: `node scripts/audit-tracks.mjs`. Importers get auditTracks() and no output.
