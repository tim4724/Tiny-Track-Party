...
import { seatGrid } from './uiModel.js';
...
//
// WHICH seats and WHAT each one holds is uiModel.seatGrid's (the padding rule,
// the car-pick fallback, the model wrap) — this function is markup only.
export function renderSeats(listEl, seats) {
  listEl.innerHTML = '';
  for (const p of seatGrid(seats, MAX_PLAYERS, CAR_MODELS.length)) {
