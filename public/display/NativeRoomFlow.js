...
// in order), and the few direct reads and writes the shell and its test
// surfaces make (roster list, host, phase, seeding a solo player). The mutable-record Proxy that used to live here served a kit
...
export class NativeRoomFlow {
  ...
  // discipline has to wrap it: the queued events re-fired — BEFORE the caller
  // performs the walk's effects, which reproduces the old inline order (the
