...
export const PLAYER_COLOR_NAMES = ['red', 'amber', 'green', 'blue', 'purple', 'pink', 'orange', 'cyan'];

export const DISPLAY_AR_DIMS = {
  '16x9': { w: 1920, h: 1080 },
  '21x9': { w: 2560, h: 1080 },
  '4x3':  { w: 1600, h: 1200 },
  '1x1':  { w: 1200, h: 1200 }
...
export const BROWSER_CHROME = { portrait: 120, landscape: 48 };
...
export function computeControllerDims(state) {
...
const STATE_KEY = 'tinytrack_gallery_state_v1';
const defaults = {
  displayAR: '16x9',
...
export function displayURL(state, scenario, extra) {
  var p = {
    scenario: scenario,
...
export function controllerURL(scenario, colorIdx, extra) {
  var p = {
    scenario: scenario,
    color: colorIdx
  };
...
let paused = false;
let pauseSafetyTimer = null;
const PAUSE_SAFETY_MS = 3000;
export function setLoadingPaused(p) {
  if (paused === p) return;
  paused = p;
...
// can clobber.
export function autoPauseOnHeaderFocus() {
...
export function lazyMount(cards) {
...
export function initMobileOptionsToggle() {
...
export function bindSelect(state, id, key, onChange, parse) {
...
export function bindCheckbox(state, id, key, onChange) {
  var el = document.getElementById(id);
  if (!el) return;
